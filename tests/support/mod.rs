...
pub(crate) fn zip_with_modes(entries: &[(&str, &[u8], ZipMethod, u32)]) -> Vec<u8> {
    ...
    for (path, contents, method, mode) in entries {
        ...
        for value in [20, 0, method_id, 0, 0] {
            push_u16(&mut archive, value);
        }
        for value in [crc, compressed.len() as u32, contents.len() as u32] {
            push_u32(&mut archive, value);
        }
        ...
        for value in [0x0314, 20, 0, method_id, 0, 0] {
            push_u16(&mut central, value);
        }
        for value in [crc, compressed.len() as u32, contents.len() as u32] {
            push_u32(&mut central, value);
        }
        for value in [path.len() as u16, 0, 0, 0, 0] {
            push_u16(&mut central, value);
        }
